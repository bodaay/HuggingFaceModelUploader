...
func main() {
	var (
		...
		SplitFileThreshhold           uint64
		IsPrivate                     bool
		install                       bool
		installPath                   string
	)
	ShortString := fmt.Sprintf("a Simple HuggingFace Models Uploader Utility\nVersion: %s", VERSION)
	...
	rootCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			err := hfu.UploadModel(ModelOrDataSet, OneFolderPerFilter, SkipSHA, IsDataset, destinationPath, branch, numberOfConcurrentConnections, HuggingFaceAccessToken)
			if err != nil {
				return err
			}
